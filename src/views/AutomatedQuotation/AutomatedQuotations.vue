    ...
    <div class="automated_wrap">
        ...
        <div class="automated_table_box">
            ...
            <div class="automated_table_content">
                <!-- 快速模具列表 -->
                <el-table v-show="navId == 'e28e6d64-d442-11e9-84f8-000c290d53bf'" class="table-list" :data="tableData4" max-height="680" row-key="numIndex" border style="width: 100%"
                    ref="multipleTable4" @selection-change="getModelSelectTrue" :span-method="objectSpanMethod" stripe>
                        <el-table-column type="selection" align='center' width="50"></el-table-column>
                        <el-table-column label="缩略图" className="drag-item" align='center' width="180">
                            <div slot-scope="scope">
                                <div class="table_pictur_box">
                                    <img v-show="scope.row.loaGif === true && scope.row.no3DImg === 0" :src="scope.row.url_img" @click="showCanvas(scope.row, 4, 'tab4')" draggable="true"
                                        @dragstart='handleDragStart($event, scope.row)'
                                        @dragover.prevent="handleDragOver($event, scope.row)"
                                        @dragenter="handleDragEnter($event, scope.row, 'tab4')"
                                        @dragend="handleDragEnd($event, scope.row, 'tab4')" />
                                    <img v-if="scope.row.loaGif === true && scope.row.no3DImg !== 0" src="../../assets/image/tp2.jpg" alt=""/>
                                    <img v-if="scope.row.loaGif === false" src="../../assets/image/loading_MN.gif" alt="">
                                </div>
                            </div>
                        </el-table-column>
                        <el-table-column label="规格" align='center' width="200">
                            <template slot-scope="scope">
                                <div class="table_3d_scope_box">
                                    <span class="table_3d_scope_span">零件名称：</span>{{scope.row.name}}
                                </div>
                                <div class="table_3d_scope_box">
                                    <span class="table_3d_scope_span">长宽高：</span>{{scope.row.long}}*{{scope.row.width}}*{{scope.row.height}}mm
                                </div>
                                <div class="table_3d_scope_box">
                                    <span class="table_3d_scope_span">体积：</span>{{scope.row.tiji}}mm
                                </div>
                                <div class="table_3d_scope_box">
                                    <span class="table_3d_scope_span">重量：</span>{{scope.row.weight}}g
                                </div>
                            </template>
                        </el-table-column>
                        <el-table-column label="参数设置" align='center' width="250">
                            <template slot-scope="scope">
                                <div class="table_3d_scope_box">
                                    <span class="table_3d_scope_span">产品结构：</span>
                                    <!-- <div>
                                        <el-select v-model="scope.row.productConst" @change="getModelproDiff(scope.row, 'tab4')" placeholder="请选择" size='mini' style="width: 150px">
                                            <el-option v-for="item in rapidStructureList" :key="item.paramContent1" :label="item.paramContent1" :value="item.paramContent1"></el-option>
                                        </el-select>
                                    </div> -->
                                    <div>
                                        <el-select v-model="scope.row.difficulty" @change="selectDiffValue(scope.row, 'tab4')" placeholder="请选择" size='mini' style="width: 150px">
                                            <el-option v-for="item in complexityList" :key="item.complexityCode" :label="item.complexityCname" :value="item.complexityCode"></el-option>
                                        </el-select>
                                    </div>
                                </div>
                                <div class="table_3d_scope_box">
                                    <span class="table_3d_scope_span">进胶类型：</span>
                                    <div>
                                        <el-select v-model="scope.row.glueType" placeholder="请选择" @change="selectGetPirce(scope.row, 'tab4')" size='mini' style="width: 150px">
                                            <el-option v-for="item in repidGumTypeList" :key="item.paramContent1" :label="item.paramContent1" :value="item.paramContent1"></el-option>
                                        </el-select>
                                    </div>
                                </div>
                                <div class="table_3d_scope_box">
                                    <span class="table_3d_scope_span">表面处理：</span>
                                    <div>
                                        <el-select v-model="scope.row.surfaceOne" @change="surfaceModel(scope.row, 'tab4')" placeholder="请选择" size='mini' style="width: 150px">
                                            <el-option key="no" label="无" value="0"></el-option>
                                            <el-option v-for="item in surface3DListOne" :key="item.finishCode" :label="item.finishCname" :value="item.finishCode"></el-option>
                                        </el-select>
                                    </div>
                                </div>
                                <div class="table_3d_scope_box">
                                    <span class="table_3d_scope_span">公差：</span>
                                    <div>
                                        <el-select v-model="scope.row.tolerance" @change="selectGetPirce(scope.row, 'tab4')" placeholder="请选择" size='mini' style="width: 150px">
                                            <el-option v-for="item in rapidToleranceList" :key="item.paramContent1" :label="item.paramContent1" :value="item.paramContent1"></el-option>
                                        </el-select>
                                    </div>
                                </div>
                            </template>
                        </el-table-column>
                        <el-table-column label="材料要求" align='center' width="250">
                            <template slot-scope="scope">
                                <div class="table_3d_scope_box">
                                    <span class="table_3d_scope_span">模仁材料：</span>
                                    <div>
                                        <el-select v-model="scope.row.moldCore" @change="getModelMR(scope.row, 'tab4')" placeholder="请选择" size='mini' style="width: 150px">
                                            <el-option v-for="item in repidModeCoreList" :key="item.id" :label="item.materialName" :value="item.materialCode"></el-option>
                                        </el-select>
                                    </div>
                                </div>
                                <div class="table_3d_scope_box">
                                    <span class="table_3d_scope_span">模坯材料：</span>
                                    <div>
                                        <el-select v-model="scope.row.dieBlank" @change="getModelMP(scope.row, 'tab4')" placeholder="请选择" size='mini' style="width: 150px">
                                            <el-option v-for="item in repidModeDieList" :key="item.materialCode" :label="item.materialName" :value="item.materialCode"></el-option>
                                        </el-select>
                                    </div>
                                </div>
                                <div class="table_3d_scope_box">
                                    <span class="table_3d_scope_span">模具寿命：</span>
                                    <div>
                                        <el-select v-model="scope.row.lifetime"  @change="selectGetPirce(scope.row, 'tab4')" placeholder="请选择" size='mini' style="width: 150px">
                                            <el-option v-for="item in repidModeDieWList" :key="item.paramContent1" :label="item.paramContent1" :value="item.paramContent1"></el-option>
                                        </el-select>
                                    </div>
                                </div>
                                <div class="table_3d_scope_box">
                                    <span class="table_3d_scope_span">交期：</span>
                                    <div>
                                        <!-- <el-select v-model="scope.row.delivery" placeholder="请选择" size='mini' style="width: 150px">
                                            <el-option v-for="item in surface3DListOne" :key="item.value" :label="item.label" :value="item.value"></el-option>
                                        </el-select> -->
                                        <el-input v-model="scope.row.delivery" maxlength="8" oninput = "value=value.replace(/[^\d.]/g,'')"
                                            size='mini' style="width: 150px" placeholder="请输入内容">
                                        </el-input>
                                    </div>
                                </div>
                            </template>
                        </el-table-column>
                        <el-table-column label="模具要求" align='center' width="250">
                            <template slot-scope="scope">
                                <div class="table_3d_scope_box">
                                    <span class="table_3d_scope_span">模坯类型：</span>
                                    <div>
                                        <el-select v-model="scope.row.dieBlankType"  @change="getCaveFun(scope.row)" placeholder="请选择" size='mini' style="width: 150px">
                                            <el-option v-for="item in repidModeTypeList" :key="item.paramContent1" :label="item.paramContent1" :value="item.paramContent1"></el-option>
                                        </el-select>
                                    </div>
                                </div>
                                <div class="table_3d_scope_box">
                                    <span class="table_3d_scope_span">行位个数：</span>
                                    <div>
                                        <el-select v-model="scope.row.rowNum"   @change="getCaveFun(scope.row)" placeholder="请选择" size='mini' style="width: 150px">
                                            <el-option v-for="item in rapidLineNumList" :key="item.paramContent1" :label="item.paramContent1" :value="item.paramContent1"></el-option>
                                        </el-select>
                                        <!-- <el-input v-model="scope.row.rowNum" maxlength="8" @change="selectGetPirce(scope.row, 'tab4')" oninput = "value=value.replace(/[^\d.]/g,'')"
                                        size='mini' style="width: 150px" placeholder="请输入内容">
                                        </el-input> -->
                                    </div>
                                </div>
                                <div class="table_3d_scope_box">
                                    <span class="table_3d_scope_span">斜顶个数：</span>
                                    <div>
                                        <el-select v-model="scope.row.tiltNum"  @change="getCaveFun(scope.row)" placeholder="请选择" size='mini' style="width: 150px">
                                            <el-option v-for="item in rapidTopNumList" :key="item.paramContent1" :label="item.paramContent1" :value="item.paramContent1"></el-option>
                                        </el-select>
                                        <!-- <el-input v-model="scope.row.tiltNum" @change="selectGetPirce(scope.row, 'tab4')" maxlength="8" oninput = "value=value.replace(/[^\d.]/g,'')"
                                        size='mini' style="width: 150px" placeholder="请输入内容">
                                        </el-input> -->
                                    </div>
                                </div>
                                <div class="table_3d_scope_box">
                                    <span class="table_3d_scope_span">手动镶件：</span>
                                    <div>
                                        <!-- <el-select v-model="scope.row.inserts" placeholder="请选择" size='mini' style="width: 150px">
                                            <el-option v-for="item in rapidHotSprueList" :key="item.value" :label="item.label" :value="item.value"></el-option>
                                        </el-select> -->
                                        <el-input v-model="scope.row.inserts"  @change="getCaveFun(scope.row)" maxlength="8" oninput = "value=value.replace(/[^\d.]/g,'')"
                                        size='mini' style="width: 150px" placeholder="请输入内容">
                                        </el-input>
                                    </div>
                                </div>
                        ...
                        <el-table-column label="其它要求" align='center' width="250">
                            <template slot-scope="scope">
                            <div class="table_3d_scope_box">
                                    <span class="table_3d_scope_span">穴数：</span>
                                    <div>
                                        <el-select v-model="scope.row.holeNum" @change="getCaveFun(scope.row)" placeholder="请选择" size='mini' style="width: 150px">
                                            <el-option v-for="item in rapidAcupointList" :key="item.paramContent1" :label="item.paramContent1" :value="item.paramContent1"></el-option>
                                        </el-select>
                                    </div>
                                </div>
                                <div class="table_3d_scope_box">
                                    <span class="table_3d_scope_span">热嘴个数：</span>
                                    <div>
                                        <el-select v-model="scope.row.hotNum" @change="getCaveFun(scope.row)" placeholder="请选择" size='mini' style="width: 150px">
                                            <el-option v-for="item in rapidHotSprueList" :key="item.paramContent1" :label="item.paramContent1" :value="item.paramContent1"></el-option>
                                        </el-select>
                                    </div>
                                </div>
                                <div class="table_3d_scope_box">
                                    <span class="table_3d_scope_span">产品材料：</span>
                                    <div>
                                        <el-select v-model="scope.row.productMaterials" placeholder="请选择" size='mini' style="width: 150px" @change='rapidSurFun(scope.row)'>
                                            <el-option v-for="item in repidProductList" :key="item.materialCode" :label="item.materialCname" :value="item.materialCode"></el-option>
                                        </el-select>
                                    </div>
                                </div>
                                <div class="table_3d_scope_box">
                                    <span class="table_3d_scope_span">产品颜色：</span>
                                    <div>
                                        <el-select v-model="scope.row.productColor" @change="getCaveFun(scope.row)" placeholder="请选择" size='mini' style="width: 150px">
                                            <el-option v-for="item in repidProductColorList" :key="item.paramContent1" :label="item.paramContent1" :value="item.paramContent1"></el-option>
                                        </el-select>
                                    </div>
                                </div>
                            </template>
                        </el-table-column>
                        <el-table-column label="产品价格" align='center' width="250">
                            <template slot-scope="scope">
                                <div class="table_3d_scope_box">
                                    <span class="table_3d_scope_span">数量：</span>
                                    <div>
                                        <el-input-number v-model="scope.row.proAmount" size='mini'  @change="selectGetPirce(scope.row, 'tab4')" :min="1" :max="10000000" label="描述文字"></el-input-number>
                                    </div>
                                </div>
                                <!-- 跟个是否为合并来显示不同的参数的价格 -->
                                <div class="table_3d_scope_box">
                                    <span class="table_3d_scope_span">产品单价：</span>
                                    <div>
                                        <el-input v-model="scope.row.proPrice" @change="editModeTotalFun(scope.row, 1)" maxlength="8" oninput = "value=value.replace(/[^\d.]/g,'')"
                                        size='mini' style="width: 150px" placeholder="请输入内容">
                                        </el-input>
                                    </div>
                                </div>
                                <div class="table_3d_scope_box">
                                    <span class="table_3d_scope_span">产品总价：</span>
                                    <div>
                                        <el-input v-model="scope.row.proTotal" @change="editModeTotalFun(scope.row, 2)" maxlength="8" oninput = "value=value.replace(/[^\d.]/g,'')"
                                        size='mini' style="width: 150px" placeholder="请输入内容">
                                        </el-input>
                                    </div>
                                </div>
                                <div class="table_3d_scope_box">
                                    <span class="table_3d_scope_span">模具总价：</span>
                                    <!-- 需要判断是否合并项 -->
                                    <div v-if="scope.row.rowFlag === ''">
                                        <el-input v-model="scope.row.moldTotal" @change="editModeTotalFun(scope.row)" maxlength="8" oninput = "value=value.replace(/[^\d.]/g,'')"
                                        size='mini' style="width: 150px" placeholder="请输入内容">
                                        </el-input>
                                    </div>
                                    <div v-if="scope.row.rowFlag !== ''">
                                        <el-input v-model="scope.row.mergeModeTotal" @change="editModeTotalFun(scope.row)" maxlength="8" oninput = "value=value.replace(/[^\d.]/g,'')"
                                        size='mini' style="width: 150px" placeholder="请输入内容">
                                        </el-input>
                                    </div>
                                </div>
                        ...
                        <!-- <el-table-column label="模具价格" align='center' width="250">
                            <template slot-scope="scope">
                                <div class="table_3d_scope_box">
                                    <span class="table_3d_scope_span">数量：</span>
                                    <div>
                                        <el-input-number v-model="scope.row.moldAmout" size='mini' @change="selectGetPirce(scope.row, 'tab4')" :min="1" :max="10000000" label="描述文字"></el-input-number>
                                    </div>
                                </div>
                                <div class="table_3d_scope_box">
                                    <span class="table_3d_scope_span">模具单价：</span>
                                    <div>
                                        <el-input v-model="scope.row.moldoPrice" maxlength="8" oninput = "value=value.replace(/[^\d.]/g,'')"
                                        size='mini' style="width: 150px" placeholder="请输入内容">
                                        </el-input>
                                    </div>
                                </div>
                                <div class="table_3d_scope_box">
                                    <span class="table_3d_scope_span">模具总价：</span>
                                    <div>
                                        <el-input v-model="scope.row.moldTotal" maxlength="8" oninput = "value=value.replace(/[^\d.]/g,'')"
                                        size='mini' style="width: 150px" placeholder="请输入内容">
                                        </el-input>
                                    </div>
                                </div>
                                <div>{{scope.row.mergeTotal + '合并后的总价'}}</div>
                                <div>{{scope.row.mergeModePrice + '合并后的模具单价'}}</div>
                                <div>{{scope.row.mergeModeTotal + '合并后的模具总价'}}</div>
                            </template>
                        </el-table-column> -->
                        <el-table-column label="总价" align='center' width="180">
                            <template slot-scope="scope">
                                {{scope.row.rowFlag === '' ? scope.row.total : scope.row.mergeTotal}}
                            </template>
                        </el-table-column>
                        ...
                        <el-table-column label="操作" align='center' width="130">
                            <template slot-scope="scope">
                                <div class="table_delete_btn hover_selec1" v-show="scope.row.btnNoshow === false" @click='deleteFun(scope.row, 4)'>
                                    <!-- <img v-if="scope.row.btnDefaFlag1 === false" src="../../assets/image/delete_def.png" alt="图片损坏"> -->
                                    <!-- <img v-if="scope.row.btnDefaFlag1 === true" src="../../assets/image/delete_on.png" alt="图片损坏"> -->
                                    <span>删除</span>
                                </div>
                                <div class="table_delete_btn hover_selec2" v-show="scope.row.btnNoshow === false" @click='copyFun(scope.row, 4)'>
                                    <!-- <img v-if="scope.row.btnDefaFlag2 === false" src="../../assets/image/fuzhi_def.png" alt="">
                                    <img v-if="scope.row.btnDefaFlag2 === true" src="../../assets/image/fuzhi_on.png" alt=""> -->
                                    <span>复制</span>
                                </div>
                                <div class="table_delete_btn hover_selec3" v-show="scope.row.btnNoshow === false"  @click='applyFun(scope.row, 4)'>
                                    <!-- <img v-if="scope.row.btnDefaFlag3 === false" src="../../assets/image/delete_def.png" alt="">
                                    <img v-if="scope.row.btnDefaFlag3 === true" src="../../assets/image/delete_on.png" alt=""> -->
                                    <span>向下应用</span>
                                </div>
                                <div class="table_delete_btn hover_selec4"  @click='uploadingFun(scope.row, "tab4")'>
                                    <!-- <img v-if="scope.row.btnDefaFlag4 === false" src="../../assets/image/uploading_def.png" alt="">
                                    <img v-if="scope.row.btnDefaFlag4 === true" src="../../assets/image/uploading_on.png" alt=""> -->
                                    <span>上传附件</span>
                                </div>
                                <div class="table_delete_btn hover_selec5"  @click='downloadFun(scope.row)'>
                                    <!-- <img v-if="scope.row.btnDefaFlag5 === false" src="../../assets/image/download_def.png" alt="">
                                    <img v-if="scope.row.btnDefaFlag5 === true" src="../../assets/image/download_on.png" alt=""> -->
                                    <span>物性表</span>
                                </div>
                            </template>
                        </el-table-column>
                ...
                <!-- 3D打印表格列表 -->
                <el-table v-show="navId == 'c4023420-d442-11e9-84f8-000c290d53bf'" :data="tableData0" max-height="680" border style="width: 100%" ref="multipleTable0"
                  @selection-change="handleSelectionChange" stripe>
                    <el-table-column type="selection" align='center' min-width="55"></el-table-column>
                    <el-table-column prop="numIndex" label="序号" align='center' min-width="60"></el-table-column>
                    <el-table-column prop="picture" label="缩略图" align='center' min-width="200">
                        <template slot-scope="scope">
                            <div class="table_pictur_box">
                                <img v-show="scope.row.loaGif === true && scope.row.no3DImg === 0" :src="scope.row.url_img" @click="showCanvas(scope.row, 0, 'tab0')" alt=暂无图片
                                    @dragstart='handleDragStart($event, scope.row)'
                                    @dragover.prevent="handleDragOver($event, scope.row)"
                                    @dragenter="handleDragEnter($event, scope.row, 'tab0')"
                                    @dragend="handleDragEnd($event, scope.row, 'tab0')" />
                                <img v-if="scope.row.loaGif === true && scope.row.no3DImg !== 0" src="../../assets/image/tp2.jpg" alt=""/>
                                <img v-if="scope.row.loaGif === false" src="../../assets/image/loading_MN.gif" alt="">
                            </div>
                        </template>
                    ...
                    <!-- 3d材料规格slot处理 -->
                    <el-table-column label="规格" align='center' min-width="280">
                        <template slot-scope="scope">
                            <div class="table_3d_scope_box">
                                <span class="table_3d_scope_span">零件名称：</span>{{scope.row.name}}
                            </div>
                            ...
                            <div class="table_3d_scope_box">
                                <span class="table_3d_scope_span">工艺：</span>
                                <div>
                                    <el-select v-model="scope.row.craft3d" placeholder="请选择" size='mini' style="width: 150px" @change='craf3dFun(scope.row)'>
                                        <el-option v-for="item in craf3DtList" :key="item.id" :label="item.techName" :value="item.id"></el-option>
                                    </el-select>
                                </div>
                            </div>
                            <div class="table_3d_scope_box">
                                <span class="table_3d_scope_span">材料：</span>
                                <div>
                                    <el-select v-model="scope.row.materials3d" placeholder="请选择" size='mini' style="width: 150px" @change='mater3dFun(scope.row)'>
                                        <el-option v-for="item in scope.row.material3DList" :key="item.materialCode" :label="item.materialCname" :value="item.materialCode"></el-option>
                                    </el-select>
                                </div>
                            </div>
                        </template>
                    </el-table-column>
                    <el-table-column label="要求" align='center' min-width="250">
                        <template slot-scope="scope">
                            <div class="table_3d_scope_box">
                                <span class="table_3d_scope_span">表面处理：</span>
                                <div>
                                    <el-select v-model="scope.row.surfaceOne" placeholder="请选择" size='mini' style="width: 150px" @change='surface3dFun(scope.row, "tab0", 1)'>
                                        <el-option key="no" label="无" value="0"></el-option>
                                        <el-option v-for="item in scope.row.surface3DListOne" :key="item.finishCode" :label="item.finishCname" :value="item.finishCode"
                                         :disabled="item.disbleFlag"></el-option>
                                    </el-select>
                                </div>
                            </div>
                            <div class="table_3d_scope_box">
                                <span class="table_3d_scope_span">表面处理：</span>
                                <div>
                                    <el-select v-model="scope.row.surfaceTwo" placeholder="请选择" size='mini' style="width: 150px" @change='surface3dFun(scope.row, "tab0", 2)'>
                                        <el-option key="no" label="无" value="0"></el-option>
                                        <el-option v-for="item in scope.row.surface3DListTwo" :key="item.finishCode" :label="item.finishCname" :value="item.finishCode"
                                         :disabled="item.disbleFlag"></el-option>
                                    </el-select>
                                </div>
                            </div>
                            <div class="table_3d_scope_box">
                                <span class="table_3d_scope_span">表面处理：</span>
                                <div>
                                    <el-select v-model="scope.row.surfaceThr" placeholder="请选择" size='mini' style="width: 150px" @change='surface3dFun(scope.row, "tab0", 3)'>
                                        <el-option key="no" label="无" value="0"></el-option>
                                        <el-option v-for="item in scope.row.surface3DListThr" :key="item.finishCode" :label="item.finishCname" :value="item.finishCode"
                                         :disabled="item.disbleFlag"></el-option>
                                    </el-select>
                                </div>
                            </div>
                    ...
                    <el-table-column label="数量" align='center' min-width="180">
                        <template slot-scope="scope">
                            <div>
                                <el-input-number v-model="scope.row.amount" size='mini' @change="amountHandleChange3d(scope.row, 'tab0')" :min="1" :max="10000000" label="描述文字"></el-input-number>
                            </div>
                        </template>
                    </el-table-column>
                    <el-table-column label="报价（RMB）" align='center' min-width="250">
                        <template slot-scope="scope">
                            <div class="table_3d_scope_box">
                                <span class="table_3d_scope_span">报价总价：</span>
                                <div>
                                    <el-input v-model="scope.row.offterTotal" maxlength="8" @change="calculatePict(scope.row, 'tab0')" oninput = "value=value.replace(/[^\d.]/g,'')"
                                      size='mini' style="width: 150px" placeholder="请输入内容">
                                    </el-input>
                                </div>
                            </div>
                            <div class="table_3d_scope_box">
                                <span class="table_3d_scope_span">报价单价：</span>
                                <div>
                                    <el-input v-model="scope.row.offterPrice" @change="editPiceFun(scope.row, 'tab0')" maxlength="8" oninput = "value=value.replace(/[^\d.]/g,'')"
                                      size='mini' style="width: 150px" placeholder="请输入内容">
                                    </el-input>
                                </div>
                            </div>
                    ...
                    <el-table-column label="评估" align='center' min-width="250">
                        <template  slot-scope="scope">
                            <div class="table_3d_scope_box">
                                <span class="table_3d_scope_span">生产周期：</span>
                                <div>
                                    <el-input v-model="scope.row.proDate" maxlength="8" oninput = "value=value.replace(/[^\d.]/g,'')"
                                      size='mini' style="width: 130px" placeholder="请输入内容">
                                    </el-input>天
                                </div>
                            </div>
                            <div class="table_3d_scope_box">
                                <span class="table_3d_scope_span">单件工时：</span>
                                <div>
                                    <el-input v-model="scope.row.proTime" maxlength="8" oninput = "value=value.replace(/[^\d.]/g,'')"
                                      size='mini' style="width: 130px" placeholder="请输入内容">
                                    </el-input>H
                                </div>
                            </div>
                            <div class="table_3d_scope_box">
                                <span class="table_3d_scope_span">手工工时：</span>
                                <div>
                                    <el-input v-model="scope.row.handicraftsTime" maxlength="8" oninput = "value=value.replace(/[^\d.]/g,'')"
                                      size='mini' style="width: 130px" placeholder="请输入内容">
                                    </el-input>H
                                </div>
                            </div>
                    ...
                    <el-table-column label="备注" align='center' min-width="197">
                        <template slot-scope="scope">
                            <div class="table_3d_scope_box">
                                <el-input
                                    type="textarea"
                    ...
                    <el-table-column label="操作" align='center' min-width="130">
                        <template slot-scope="scope">
                            <div class="table_delete_btn hover_selec1"  @click='deleteFun(scope.row, 0)'>
                                <!-- <img v-if="scope.row.btnDefaFlag1 === false" src="../../assets/image/delete_def.png" alt="图片损坏"> -->
                                <!-- <img v-if="scope.row.btnDefaFlag1 === true" src="../../assets/image/delete_on.png" alt="图片损坏"> -->
                                <span>删除</span>
                            </div>
                            <div class="table_delete_btn hover_selec2"  @click='copyFun(scope.row, 0)'>
                                <!-- <img v-if="scope.row.btnDefaFlag2 === false" src="../../assets/image/fuzhi_def.png" alt="">
                                <img v-if="scope.row.btnDefaFlag2 === true" src="../../assets/image/fuzhi_on.png" alt=""> -->
                                <span>复制</span>
                            </div>
                            <div class="table_delete_btn hover_selec3"  @click='applyFun(scope.row, 0)'>
                                <!-- <img v-if="scope.row.btnDefaFlag3 === false" src="../../assets/image/delete_def.png" alt="">
                                <img v-if="scope.row.btnDefaFlag3 === true" src="../../assets/image/delete_on.png" alt=""> -->
                                <span>向下应用</span>
                            </div>
                            <div class="table_delete_btn hover_selec4"  @click='uploadingFun(scope.row, "tab0")'>
                                <!-- <img v-if="scope.row.btnDefaFlag4 === false" src="../../assets/image/uploading_def.png" alt="">
                                <img v-if="scope.row.btnDefaFlag4 === true" src="../../assets/image/uploading_on.png" alt=""> -->
                                <span>上传附件</span>
                            </div>
                ...
                <!-- CNC金属表格列表 -->
                <el-table v-show="navId == 'e27ad2ee-d442-11e9-84f8-000c290d53bf'" class="table-list" :data="tableData1" max-height="680" border style="width: 100%" ref="multipleTable1"
                    @selection-change="handleSelectionChange1" stripe>
                    <el-table-column type="selection" align='center' min-width="55"></el-table-column>
                    <el-table-column prop="numIndex" label="序号" align='center' min-width="60"></el-table-column>
                    <el-table-column label="图片" align='center' min-width="150">
                        <template slot-scope="scope">
                            <div class="table_pictur_box">
                                <img v-show="scope.row.loaGif === true && scope.row.no3DImg === 0" :src="scope.row.url_img" @click="showCanvas(scope.row, 1, 'tab1')" alt=暂无图片
                                    @dragstart='handleDragStart($event, scope.row)'
                                    @dragover.prevent="handleDragOver($event, scope.row)"
                                    @dragenter="handleDragEnter($event, scope.row, 'tab1')"
                                    @dragend="handleDragEnd($event, scope.row, 'tab1')" />
                                <img v-if="scope.row.loaGif === true && scope.row.no3DImg !== 0" src="../../assets/image/tp2.jpg" alt=""/>
                                <img v-if="scope.row.loaGif === false" src="../../assets/image/loading_MN.gif" alt="">
                            </div>
                        </template>
                    </el-table-column>
                    <el-table-column label="规格" align='center' min-width="250">
                        <template slot-scope="scope">
                            <div class="table_3d_scope_box">
                                <span class="table_3d_scope_span">零件名称：</span>
                                {{scope.row.name}}
                            </div>
                            <div class="table_3d_scope_box">
                                <span class="table_3d_scope_span">长宽高：</span>
                                {{scope.row.long}}*{{scope.row.width}}*{{scope.row.height}}mm
                            </div>
                            <div class="table_3d_scope_box">
                                <span class="table_3d_scope_span">重量：</span>
                                {{scope.row.weight}}g
                            </div>
                            <div class="table_3d_scope_box">
                                <span class="table_3d_scope_span">工艺：</span>
                                <div>
                                    <el-select v-model="scope.row.craft3d" placeholder="请选择" size='mini' style="width: 150px" @change='snsCrafFun(scope.row, 0, "tab1")'>
                                        <el-option v-for="item in snsCrafList" :key="item.id" :label="item.techName" :value="item.id"></el-option>
                                    </el-select>
                                </div>
                            </div>
                            <div class="table_3d_scope_box">
                                <span class="table_3d_scope_span">材料：</span>
                                <div>
                                    <el-select v-model="scope.row.materials3d" placeholder="请选择" size='mini' style="width: 150px" @change="snsMaterFun(scope.row, 'tab1')">
                                        <el-option v-for="item in scope.row.snsMaterralList" :key="item.materialCode" :label="item.materialCname" :value="item.materialCode"></el-option>
                                    </el-select>
                                </div>
                            </div>
                        </template>
                    </el-table-column>
                    <el-table-column label="参数" align='center' min-width="250">
                        <template slot-scope="scope">
                            <div class="table_3d_scope_box">
                                <span class="table_3d_scope_span">拆件：</span>
                                <div>
                                    <el-select v-model="scope.row.piecemeal" placeholder="请选择" @change="selectGetPirce(scope.row, 'tab1')" size='mini' style="width: 150px">
                                        <el-option v-for="item in snsMetalPulgList" :key="item.paramContent1" :label="item.paramContent1" :value="item.paramContent1"></el-option>
                                    </el-select>
                                </div>
                            </div>
                            <div class="table_3d_scope_box">
                                <span class="table_3d_scope_span">加工面数：</span>
                                <div>
                                    <el-select v-model="scope.row.machinePage" placeholder="请选择" @change="selectGetPirce(scope.row, 'tab1')" size='mini' style="width: 150px">
                                        <el-option v-for="item in snsMetalPage" :key="item.paramContent1" :label="item.paramContent1" :value="item.paramContent1"></el-option>
                                    </el-select>
                                </div>
                            </div>
                            <div class="table_3d_scope_box">
                                <span class="table_3d_scope_span">难度系数：</span>
                                <div>
                                    <el-select v-model="scope.row.difficulty" @change="selectDiffValue(scope.row, 'tab1')" placeholder="请选择" size='mini' style="width: 150px">
                                        <el-option v-for="item in complexityList" :key="item.complexityCode" :label="item.complexityCname" :value="item.complexityCode"></el-option>
                                    </el-select>
                                </div>
                            </div>
                            <div class="table_3d_scope_box">
                                <span class="table_3d_scope_span">攻牙：</span>
                                <div>
                                    <!-- <el-select v-model="scope.row.tapping" placeholder="请选择" size='mini' style="width: 150px">
                                        <el-option v-for="item in craf3DtList" :key="item.value" :label="item.label" :value="item.value"></el-option>
                                    </el-select> -->
                                    <el-input v-model="scope.row.tapping" @change="selectGetPirce(scope.row, 'tab1')" maxlength="999" oninput = "value=value.replace(/[^\d]/g,'')"
                                        size='mini' style="width: 80px" placeholder="请输入内容">
                                    </el-input>
                                </div>
                            </div>
                            <div class="table_3d_scope_box">
                                <span class="table_3d_scope_span">公差：</span>
                                <div>
                                    <el-select v-model="scope.row.tolerance" @change="selectGetPirce(scope.row, 'tab1')" placeholder="请选择" size='mini' style="width: 150px">
                                        <el-option v-for="item in snsMetalToteranList" :key="item.paramContent1" :label="item.paramContent1" :value="item.paramContent1"></el-option>
                                    </el-select>
                                </div>
                            </div>
                        </template>
                    ...
                    <el-table-column label="要求" align='center' min-width="250">
                        <template slot-scope="scope">
                            <div class="table_3d_scope_box">
                                <el-checkbox-group v-model="scope.row.sparkList">
                                    <el-checkbox v-for="city in cncArr" :label="city.title" :key="city.val">{{city.title}}</el-checkbox>
                                </el-checkbox-group>
                            </div>
                            <div class="table_3d_scope_box">
                                <span class="table_3d_scope_span">嵌件：</span>
                                <div>
                                    <!-- <el-select v-model="scope.row.inserts" placeholder="请选择" size='mini' style="width: 150px">
                                        <el-option v-for="item in craf3DtList" :key="item.value" :label="item.label" :value="item.value"></el-option>
                                    </el-select> -->
                                    <el-input v-model="scope.row.inserts" @change="selectGetPirce(scope.row, 'tab1')" maxlength="999" oninput = "value=value.replace(/[^\d]/g,'')"
                                        size='mini' style="width: 80px" placeholder="请输入内容">
                                    </el-input>
                                </div>
                            </div>
                            <div class="table_3d_scope_box">
                                <span class="table_3d_scope_span">表面处理：</span>
                                <div>
                                    <el-select v-model="scope.row.surfaceOne" placeholder="请选择" size='mini' style="width: 150px" @change='snsJSurfaceFun(scope.row)'>
                                        <el-option key="no" label="无" value="0"></el-option>
                                        <el-option v-for="item in scope.row.surface3DListOne" :key="item.finishCode" :label="item.finishCname" :value="item.finishCode"
                                        :disabled="item.disbleFlag"></el-option>
                                    </el-select>
                            ...
                            <div class="table_3d_scope_box">
                                <span class="table_3d_scope_span">表面处理：</span>
                                <div>
                                    <el-select v-model="scope.row.surfaceTwo" placeholder="请选择" size='mini' style="width: 150px" @change='snsJSurfaceFun(scope.row)'>
                                        <el-option key="no" label="无" value="0"></el-option>
                                        <el-option v-for="item in scope.row.surface3DListTwo" :key="item.finishCode" :label="item.finishCname" :value="item.finishCode"
                                        :disabled="item.disbleFlag"></el-option>
                                    </el-select>
                            ...
                            <div class="table_3d_scope_box">
                                <span class="table_3d_scope_span">表面处理：</span>
                                <div>
                                    <el-select v-model="scope.row.surfaceThr" placeholder="请选择" size='mini' style="width: 150px" @change='snsJSurfaceFun(scope.row)'>
                                        <el-option key="no" label="无" value="0"></el-option>
                                        <el-option v-for="item in scope.row.surface3DListThr" :key="item.finishCode" :label="item.finishCname" :value="item.finishCode"
                                        :disabled="item.disbleFlag"></el-option>
                                    </el-select>
                    ...
                    <el-table-column label="数量" align='center' min-width="150">
                        <template slot-scope="scope">
                            <div>
                                <el-input-number v-model="scope.row.amount" size='mini' @change="selectGetPirce(scope.row, 'tab1')" :min="1" :max="10000000" label="描述文字"></el-input-number>
                            </div>
                        </template>
                    </el-table-column>
                    <el-table-column label="报价（RMB）" align='center' min-width="235">
                        <template slot-scope="scope">
                            <div class="table_3d_scope_box">
                                <span class="table_3d_scope_span">报价总价：</span>
                                <div>
                                    <el-input v-model="scope.row.offterTotal" @change="calculatePict(scope.row, 'tab1')" maxlength="8" oninput = "value=value.replace(/[^\d.]/g,'')"
                                        size='mini' style="width: 120px" placeholder="请输入内容">
                                    </el-input>
                                </div>
                            </div>
                            <div class="table_3d_scope_box">
                                <span class="table_3d_scope_span">报价单价：</span>
                                <div>
                                    <el-input v-model="scope.row.offterPrice" @change="editPiceFun(scope.row, 'tab1')" maxlength="8" oninput = "value=value.replace(/[^\d.]/g,'')"
                                        size='mini' style="width: 120px" placeholder="请输入内容">
                                    </el-input>
                                </div>
                            </div>
                    ...
                    <el-table-column label="评估" align='center' min-width="192">
                        <template  slot-scope="scope">
                            <div class="table_3d_scope_box">
                                <span class="table_3d_scope_span">生产周期：</span>
                                <div>
                                    <el-input v-model="scope.row.proDate" maxlength="6" oninput = "value=value.replace(/[^\d.]/g,'')"
                                    size='mini' style="width: 80px" placeholder="请输入内容">
                                    </el-input>天
                                </div>
                            </div>
                            <div class="table_3d_scope_box">
                                <span class="table_3d_scope_span">单件工时：</span>
                                <div>
                                    <el-input v-model="scope.row.proTime" maxlength="6" oninput = "value=value.replace(/[^\d.]/g,'')"
                                    size='mini' style="width: 80px" placeholder="请输入内容">
                                    </el-input>H
                                </div>
                            </div>
                            <div class="table_3d_scope_box">
                                <span class="table_3d_scope_span">手工工时：</span>
                                <div>
                                    <el-input v-model="scope.row.handicraftsTime" maxlength="6" oninput = "value=value.replace(/[^\d.]/g,'')"
                                    size='mini' style="width: 80px" placeholder="请输入内容">
                                    </el-input>H
                                </div>
                            </div>
                    ...
                    <el-table-column label="操作" align='center' min-width="110">
                            <template slot-scope="scope">
                            <div class="table_delete_btn hover_selec1"  @click='deleteFun(scope.row, 1)'>
                                    <!-- <img v-if="scope.row.btnDefaFlag1 === false" src="../../assets/image/delete_def.png" alt="图片损坏"> -->
                                    <!-- <img v-if="scope.row.btnDefaFlag1 === true" src="../../assets/image/delete_on.png" alt="图片损坏"> -->
                                    <span>删除</span>
                                </div>
                                <div class="table_delete_btn hover_selec2"  @click='copyFun(scope.row, 1)'>
                                    <!-- <img v-if="scope.row.btnDefaFlag2 === false" src="../../assets/image/fuzhi_def.png" alt="">
                                    <img v-if="scope.row.btnDefaFlag2 === true" src="../../assets/image/fuzhi_on.png" alt=""> -->
                                    <span>复制</span>
                                </div>
                                <div class="table_delete_btn hover_selec3"  @click='applyFun(scope.row, 1)'>
                                    <!-- <img v-if="scope.row.btnDefaFlag3 === false" src="../../assets/image/delete_def.png" alt="">
                                    <img v-if="scope.row.btnDefaFlag3 === true" src="../../assets/image/delete_on.png" alt=""> -->
                                    <span>向下应用</span>
                                </div>
                                <div class="table_delete_btn hover_selec4"  @click='uploadingFun(scope.row, "tab1")'>
                                    <!-- <img v-if="scope.row.btnDefaFlag4 === false" src="../../assets/image/uploading_def.png" alt="">
                                    <img v-if="scope.row.btnDefaFlag4 === true" src="../../assets/image/uploading_on.png" alt=""> -->
                                    <span>上传附件</span>
                                </div>
                ...
                <!-- CNC塑料表格列表 -->
                <el-table v-show="navId == 'e2828bcb-d442-11e9-84f8-000c290d53bf'" :data="tableData2" max-height="680" border style="width: 100%" ref="multipleTable2"
                  @selection-change="handleSelectionChange2" stripe>
                    <el-table-column type="selection" align='center' min-width="55"></el-table-column>
                    <el-table-column prop="numIndex" label="序号" align='center' min-width="60"></el-table-column>
                    <el-table-column label="图片" align='center' min-width="150">
                         <template slot-scope="scope">
                            <div class="table_pictur_box">
                                <img v-show="scope.row.loaGif === true && scope.row.no3DImg === 0" :src="scope.row.url_img" @click="showCanvas(scope.row, 2, 'tab2')" alt=暂无图片
                                    @dragstart='handleDragStart($event, scope.row)'
                                    @dragover.prevent="handleDragOver($event, scope.row)"
                                    @dragenter="handleDragEnter($event, scope.row, 'tab2')"
                                    @dragend="handleDragEnd($event, scope.row, 'tab2')" />
                                <img v-if="scope.row.loaGif === true && scope.row.no3DImg !== 0" src="../../assets/image/tp2.jpg" alt=""/>
                                <img v-if="scope.row.loaGif === false" src="../../assets/image/loading_MN.gif" alt="">
                            </div>
                        </template>
                    ...
                    <el-table-column label="规格" align='center' min-width="250">
                        <template slot-scope="scope">
                            ...
                            <div class="table_3d_scope_box">
                                <span class="table_3d_scope_span">工艺：</span>
                                <div>
                                    <el-select v-model="scope.row.craft3d" placeholder="请选择" size='mini' style="width: 150px" @change='snsCrafFun(scope.row, 1, "tab2")'>
                                        <el-option v-for="item in snsCrafList" :key="item.id" :label="item.techName" :value="item.id"></el-option>
                                    </el-select>
                                </div>
                            </div>
                            <div class="table_3d_scope_box">
                                <span class="table_3d_scope_span">材料：</span>
                                <div>
                                    <el-select v-model="scope.row.materials3d" placeholder="请选择" size='mini' style="width: 150px" @change="snsMaterFun(scope.row, 'tab2')">
                                        <el-option v-for="item in scope.row.snsMaterralList" :key="item.materialCode" :label="item.materialCname" :value="item.materialCode"></el-option>
                                    </el-select>
                                </div>
                            </div>
                    ...
                    <el-table-column label="参数" align='center' min-width="250">
                        <template slot-scope="scope">
                            <div class="table_3d_scope_box">
                                <span class="table_3d_scope_span">拆件：</span>
                                <div>
                                    <el-select v-model="scope.row.piecemeal" @change="selectGetPirce(scope.row, 'tab2')" placeholder="请选择" size='mini' style="width: 150px">
                                        <el-option v-for="item in snsPlasPulgList" :key="item.paramContent1" :label="item.paramContent1" :value="item.paramContent1"></el-option>
                                    </el-select>
                                </div>
                            </div>
                            <div class="table_3d_scope_box">
                                <span class="table_3d_scope_span">加工面数：</span>
                                <div>
                                    <el-select v-model="scope.row.machinePage" @change="selectGetPirce(scope.row, 'tab2')" placeholder="请选择" size='mini' style="width: 150px">
                                        <el-option v-for="item in snsPlasPageList" :key="item.paramContent1" :label="item.paramContent1" :value="item.paramContent1"></el-option>
                                    </el-select>
                                </div>
                            </div>
                            <div class="table_3d_scope_box">
                                <span class="table_3d_scope_span">难度系数：</span>
                                <div>
                                    <el-select v-model="scope.row.difficulty" @change="selectDiffValue(scope.row, 'tab2')" placeholder="请选择" size='mini' style="width: 150px">
                                        <el-option v-for="item in complexityList" :key="item.complexityCode" :label="item.complexityCname" :value="item.complexityCode"></el-option>
                                    </el-select>
                                </div>
                            </div>
                            <div class="table_3d_scope_box">
                                ...
                                <div>
                                    ...
                                    <el-input v-model="scope.row.tapping" @change="selectGetPirce(scope.row, 'tab2')" maxlength="999" oninput = "value=value.replace(/[^\d]/g,'')"
                                        size='mini' style="width: 80px" placeholder="请输入内容">
                                    </el-input>
                                </div>
                            </div>
                            <div class="table_3d_scope_box">
                                <span class="table_3d_scope_span">公差：</span>
                                <div>
                                    <el-select v-model="scope.row.tolerance" @change="selectGetPirce(scope.row, 'tab2')" placeholder="请选择" size='mini' style="width: 150px">
                                        <el-option v-for="item in snsPlastoleranceList" :key="item.paramContent1" :label="item.paramContent1" :value="item.paramContent1"></el-option>
                                    </el-select>
                                </div>
                            </div>
                        </template>
                        ...
                        <template slot-scope="scope">
                            <div class="table_3d_scope_box">
                                ...
                                <div>
                                    ...
                                    <el-input v-model="scope.row.inserts" maxlength="999" oninput = "value=value.replace(/[^\d]/g,'')"
                                        size='mini' style="width: 80px" placeholder="请输入内容">
                                    </el-input>
                                </div>
                            </div>
                             <div class="table_3d_scope_box">
                                <span class="table_3d_scope_span">表面处理：</span>
                                <div>
                                    <el-select v-model="scope.row.surfaceOne" placeholder="请选择" size='mini' style="width: 150px" @change='snsSSurfaceFun(scope.row)'>
                                        <el-option key="no" label="无" value="0"></el-option>
                                        <el-option v-for="item in scope.row.surface3DListOne" :key="item.finishCode" :label="item.finishCname" :value="item.finishCode"
                                         :disabled="item.disbleFlag"></el-option>
                                    </el-select>
                            ...
                            <div class="table_3d_scope_box">
                                <span class="table_3d_scope_span">表面处理：</span>
                                <div>
                                    <el-select v-model="scope.row.surfaceTwo" placeholder="请选择" size='mini' style="width: 150px" @change='snsSSurfaceFun(scope.row)'>
                                        <el-option key="no" label="无" value="0"></el-option>
                                        <el-option v-for="item in scope.row.surface3DListTwo" :key="item.finishCode" :label="item.finishCname" :value="item.finishCode"
                                         :disabled="item.disbleFlag"></el-option>
                                    </el-select>
                            ...
                            <div class="table_3d_scope_box">
                                <span class="table_3d_scope_span">表面处理：</span>
                                <div>
                                    <el-select v-model="scope.row.surfaceThr" placeholder="请选择" size='mini' style="width: 150px" @change='snsSSurfaceFun(scope.row)'>
                                        <el-option key="no" label="无" value="0"></el-option>
                                        <el-option v-for="item in scope.row.surface3DListThr" :key="item.finishCode" :label="item.finishCname" :value="item.finishCode"
                                         :disabled="item.disbleFlag"></el-option>
                                    </el-select>
                    ...
                    <el-table-column label="数量" align='center' min-width="150">
                         <template slot-scope="scope">
                            <div>
                                <el-input-number v-model="scope.row.amount" size='mini' @change="selectGetPirce(scope.row, 'tab2')" :min="1" :max="10000000" label="描述文字">
                                </el-input-number>
                            </div>
                        </template>
                    </el-table-column>
                    <el-table-column label="报价（RMB）" align='center' min-width="235">
                        <template slot-scope="scope">
                            <div class="table_3d_scope_box">
                                <span class="table_3d_scope_span">报价总价：</span>
                                <div>
                                    <el-input v-model="scope.row.offterTotal" maxlength="8" @change="calculatePict(scope.row, 'tab2')" oninput = "value=value.replace(/[^\d.]/g,'')"
                                        size='mini' style="width: 120px" placeholder="请输入内容">
                                    </el-input>
                                </div>
                            </div>
                            <div class="table_3d_scope_box">
                                <span class="table_3d_scope_span">报价单价：</span>
                                <div>
                                    <el-input v-model="scope.row.offterPrice" maxlength="8" @change="editPiceFun(scope.row, 'tab2')" oninput = "value=value.replace(/[^\d.]/g,'')"
                                        size='mini' style="width: 120px" placeholder="请输入内容">
                                    </el-input>
                                </div>
                            </div>
                    ...
                    <el-table-column label="操作" align='center' min-width="110">
                        <template slot-scope="scope">
                            <div class="table_delete_btn hover_selec1"  @click='deleteFun(scope.row, 2)'>
                                <!-- <img v-if="scope.row.btnDefaFlag1 === false" src="../../assets/image/delete_def.png" alt="图片损坏"> -->
                                <!-- <img v-if="scope.row.btnDefaFlag1 === true" src="../../assets/image/delete_on.png" alt="图片损坏"> -->
                                <span>删除</span>
                            </div>
                            <div class="table_delete_btn hover_selec2"  @click='copyFun(scope.row, 2)'>
                                <!-- <img v-if="scope.row.btnDefaFlag2 === false" src="../../assets/image/fuzhi_def.png" alt="">
                                <img v-if="scope.row.btnDefaFlag2 === true" src="../../assets/image/fuzhi_on.png" alt=""> -->
                                <span>复制</span>
                            </div>
                            <div class="table_delete_btn hover_selec3"  @click='applyFun(scope.row, 2)'>
                                <!-- <img v-if="scope.row.btnDefaFlag3 === false" src="../../assets/image/delete_def.png" alt="">
                                <img v-if="scope.row.btnDefaFlag3 === true" src="../../assets/image/delete_on.png" alt=""> -->
                                <span>向下应用</span>
                            </div>
                            <div class="table_delete_btn hover_selec4"  @click='uploadingFun(scope.row, "tab2")'>
                                <!-- <img v-if="scope.row.btnDefaFlag4 === false" src="../../assets/image/uploading_def.png" alt="">
                                <img v-if="scope.row.btnDefaFlag4 === true" src="../../assets/image/uploading_on.png" alt=""> -->
                                <span>上传附件</span>
                            </div>
                ...
                <!-- 复模表格列表 -->
                <el-table v-show="navId == 'e287c643-d442-11e9-84f8-000c290d53bf'" :data="tableData3" max-height="680" border style="width: 100%" ref="multipleTable3"
                  @selection-change="handleSelectionChange3" stripe>
                    <el-table-column type="selection" align='center' min-width="55"></el-table-column>
                    <el-table-column prop="numIndex" label="序号" align='center' min-width="60"></el-table-column>
                    <el-table-column label="图片" align='center' min-width="150">
                         <template slot-scope="scope">
                            <div class="table_pictur_box">
                                <img v-show="scope.row.loaGif === true && scope.row.no3DImg === 0" :src="scope.row.url_img" @click="showCanvas(scope.row, 3, 'tab3')" alt=暂无图片
                                    @dragstart='handleDragStart($event, scope.row)'
                                    @dragover.prevent="handleDragOver($event, scope.row)"
                                    @dragenter="handleDragEnter($event, scope.row, 'tab3')"
                                    @dragend="handleDragEnd($event, scope.row, 'tab3')" />
                                <img v-if="scope.row.loaGif === true && scope.row.no3DImg !== 0" src="../../assets/image/tp2.jpg" alt=""/>
                                <img v-if="scope.row.loaGif === false" src="../../assets/image/loading_MN.gif" alt="">
                            </div>
                        </template>
                    ...
                    <el-table-column label="规格" align='center' min-width="250">
                        <template slot-scope="scope">
                            ...
                            <div class="table_3d_scope_box">
                                <span class="table_3d_scope_span">工艺：</span>
                                <div>
                                    <el-select v-model="scope.row.craft3d" placeholder="请选择" size='mini' style="width: 150px" @change="modeCrafFun(scope.row, 'tab3')">
                                        <el-option v-for="item in modeCrafList" :key="item.id" :label="item.techName" :value="item.id"></el-option>
                                    </el-select>
                                </div>
                            </div>
                            <div class="table_3d_scope_box">
                                <span class="table_3d_scope_span">灌注材料：</span>
                                <div>
                                    <el-select v-model="scope.row.materials3d" @change="getFuMuPour(scope.row, 'tab3')" placeholder="请选择" size='mini' style="width: 150px">
                                        <el-option v-for="item in modePourList" :key="item.materialCode" :label="item.materialCname" :value="item.materialCode"></el-option>
                                    </el-select>
                                </div>
                            </div>
                        </template>
                    ...
                    <el-table-column label="参数" align='center' min-width="250">
                        <template slot-scope="scope">
                            <div class="table_3d_scope_box">
                                <span class="table_3d_scope_span" >原型材料：</span>
                                <div>
                                    <el-select v-model="scope.row.promaterials" placeholder="请选择" size='mini' style="width: 150px" @change="modeProtypeMaterFun(scope.row, 'tab3')">
                                        <el-option v-for="item in scope.row.promaterialsLsit" :key="item.materialCode" :label="item.materialCname" :value="item.materialCode"></el-option>
                                    </el-select>
                                </div>
                            </div>
                            <div class="table_3d_scope_box">
                                <span class="table_3d_scope_span">硅模材料：</span>
                                <div>
                                    <el-select v-model="scope.row.Silicon" @change="getSiliconName(scope.row, 'tab3')" placeholder="请选择" size='mini' style="width: 150px">
                                        <el-option v-for="item in siliconModeList" :key="item.materialCode" :label="item.materialCname" :value="item.materialCode"></el-option>
                                    </el-select>
                                </div>
                            </div>
                            <div class="table_3d_scope_box">
                                <span class="table_3d_scope_span">难度系数：</span>
                                <div>
                                    <el-select v-model="scope.row.difficulty" @change="selectDiffValue(scope.row, 'tab3')"  placeholder="请选择" size='mini' style="width: 150px">
                                        <el-option v-for="item in complexityList" :key="item.complexityCode" :label="item.complexityCname" :value="item.complexityCode"></el-option>
                                    </el-select>
                                </div>
                            </div>
                            <div class="table_3d_scope_box">
                                ...
                                <div>
                                    ...
                                    <el-input v-model="scope.row.tapping" @change="selectGetPirce(scope.row, 'tab3')" maxlength="999" oninput = "value=value.replace(/[^\d]/g,'')"
                                        size='mini' style="width: 80px" placeholder="请输入内容">
                                    </el-input>
                                </div>
                            </div>
                            <div class="table_3d_scope_box">
                                <span class="table_3d_scope_span">公差：</span>
                                <div>
                                    <el-select v-model="scope.row.tolerance" @change="selectGetPirce(scope.row, 'tab3')" placeholder="请选择" size='mini' style="width: 150px">
                                        <el-option v-for="item in modeToleranceList" :key="item.paramContent1" :label="item.paramContent1" :value="item.paramContent1"></el-option>
                                    </el-select>
                                </div>
                            </div>
                        </template>
                    ...
                    <el-table-column label="要求" align='center' min-width="250">
                        <template slot-scope="scope">
                            <div class="table_3d_scope_box">
                                <span class="table_3d_scope_span">表面处理：</span>
                                <div>
                                    <el-select v-model="scope.row.surfaceOne" placeholder="请选择" size='mini' style="width: 150px" @change='modeSurfaceFun(scope.row, "tab3")'>
                                        <el-option key="no" label="无" value="0"></el-option>
                                        <el-option v-for="item in scope.row.surface3DListOne" :key="item.finishCode" :label="item.finishCname" :value="item.finishCode"
                                         :disabled="item.disbleFlag"></el-option>
                                    </el-select>
                            ...
                            <div class="table_3d_scope_box">
                                <span class="table_3d_scope_span">表面处理：</span>
                                <div>
                                    <el-select v-model="scope.row.surfaceTwo" placeholder="请选择" size='mini' style="width: 150px" @change='modeSurfaceFun(scope.row, "tab3")'>
                                        <el-option key="no" label="无" value="0"></el-option>
                                        <el-option v-for="item in scope.row.surface3DListTwo" :key="item.finishCode" :label="item.finishCname" :value="item.finishCode"
                                         :disabled="item.disbleFlag"></el-option>
                                    </el-select>
                            ...
                            <div class="table_3d_scope_box">
                                <span class="table_3d_scope_span">表面处理：</span>
                                <div>
                                    <el-select v-model="scope.row.surfaceThr" placeholder="请选择" size='mini' style="width: 150px" @change='modeSurfaceFun(scope.row, "tab3")'>
                                        <el-option key="no" label="无" value="0"></el-option>
                                        <el-option v-for="item in scope.row.surface3DListThr" :key="item.finishCode" :label="item.finishCname" :value="item.finishCode"
                                            :disabled="item.disbleFlag"></el-option>
                                    </el-select>
                    ...
                    <el-table-column label="数量" align='center' min-width="150">
                         <template slot-scope="scope">
                            <div>
                                <el-input-number v-model="scope.row.amount" size='mini' @change="selectGetPirce(scope.row, 'tab3')" :min="1" :max="10000000" label="描述文字">
                                </el-input-number>
                            </div>
                        </template>
                    </el-table-column>
                    <el-table-column label="报价（RMB）" align='center' min-width="235">
                        <template slot-scope="scope">
                            <div class="table_3d_scope_box">
                                <span class="table_3d_scope_span">原型总价：</span>
                                <div>
                                    <el-input v-model="scope.row.proTotal" @change="getFuMoTotal(scope.row)" maxlength="8" oninput = "value=value.replace(/[^\d.]/g,'')"
                                        size='mini' style="width: 120px" placeholder="请输入内容">
                                    </el-input>
                                </div>
                            </div>
                            <div class="table_3d_scope_box">
                                <span class="table_3d_scope_span">模具总价：</span>
                                <div>
                                    <el-input v-model="scope.row.moldTotal" @change="getFuMoTotal(scope.row)" maxlength="8" oninput = "value=value.replace(/[^\d.]/g,'')"
                                        size='mini' style="width: 120px" placeholder="请输入内容">
                                    </el-input>
                                </div>
                            </div>
                            <div class="table_3d_scope_box">
                                <span class="table_3d_scope_span">灌注单价：</span>
                                <div>
                                    <el-input v-model="scope.row.pourPrice" maxlength="8" oninput = "value=value.replace(/[^\d.]/g,'')"
                                        size='mini' style="width: 120px" @change="getFuMoTotal(scope.row)" placeholder="请输入内容">
                                    </el-input>
                                </div>
                            </div>
                            <div class="table_3d_scope_box">
                                <span class="table_3d_scope_span">报价总价：</span>
                                <div>
                                    <el-input v-model="scope.row.offterTotal" disabled="" maxlength="8" oninput = "value=value.replace(/[^\d.]/g,'')"
                                        size='mini' style="width: 120px" placeholder="请输入内容">
                                    </el-input>
                                </div>
                            </div>
                    ...
                    <el-table-column label="操作" align='center' min-width="110">
                        <template slot-scope="scope">
                           <div class="table_delete_btn hover_selec1"  @click='deleteFun(scope.row, 3)'>
                                <!-- <img v-if="scope.row.btnDefaFlag1 === false" src="../../assets/image/delete_def.png" alt="图片损坏"> -->
                                <!-- <img v-if="scope.row.btnDefaFlag1 === true" src="../../assets/image/delete_on.png" alt="图片损坏"> -->
                                <span>删除</span>
                            </div>
                            <div class="table_delete_btn hover_selec2"  @click='copyFun(scope.row, 3)'>
                                <!-- <img v-if="scope.row.btnDefaFlag2 === false" src="../../assets/image/fuzhi_def.png" alt="">
                                <img v-if="scope.row.btnDefaFlag2 === true" src="../../assets/image/fuzhi_on.png" alt=""> -->
                                <span>复制</span>
                            </div>
                            <div class="table_delete_btn hover_selec3"  @click='applyFun(scope.row, 3)'>
                                <!-- <img v-if="scope.row.btnDefaFlag3 === false" src="../../assets/image/delete_def.png" alt="">
                                <img v-if="scope.row.btnDefaFlag3 === true" src="../../assets/image/delete_on.png" alt=""> -->
                                <span>向下应用</span>
                            </div>
                            <div class="table_delete_btn hover_selec4"  @click='uploadingFun(scope.row, "tab3")'>
                                <!-- <img v-if="scope.row.btnDefaFlag4 === false" src="../../assets/image/uploading_def.png" alt="">
                                <img v-if="scope.row.btnDefaFlag4 === true" src="../../assets/image/uploading_on.png" alt=""> -->
                                <span>上传附件</span>
                            </div>
                ...
                <div class="footer_total">
                    <el-checkbox v-model="checked" @change="getAllListTotal">全选</el-checkbox>
                    <div>
                        总价：<span class="orgin_color">{{money}}</span>
                    </div>
                </div>
            </div>
        </div>
        <!--3d弹窗集合-->
        <div class="threedwindow" :class="{'showwindow':threeboxflag}">
            <div class="titleBox">
                <span class="text">3D模型预览</span>
                <span class="close" @click="closewindow">×</span>
            </div>
            <div class="screenshotBtnBox">
                <button @click="editImg">截图</button>
                <div class="canvasName " style="display:none;"></div>
            </div>
            <!--存放3d模型样式-->
            <div id="abc" ref="canvasBox" :class="{'showwindow':!threeboxflag}">
            <!-- <button :class="imgIndex" @click="editImg">截图</button> -->
            <!--<p>666</p>-->
            </div>
        </div>
        <el-dialog title="无3D图纸报价"
...
export default {
    ...
    created () {
        ...
        var setInNum = setInterval(() => {
            if (this.editDataFlag == 0) {
                ...
                // this.$router.push({ name: 'quotation', params: { params: row } });this.$route.params.orderNumber

                // console.log("*********************");
                // let mainId=this.$router.params.quoteRow;
                // console.log(mainId)
    ...
    methods: {
                    ...
                    if (type === 'tab0') { // 3D打印
                        ...
                        api.getThrPirceFun(objV).then(res => {
                            ...
                            if (res.code === 0) {
                                ...
                                /*****罗丹珠加----开始 */
                                list[k].cusTotalpartmaterialCost=res.obj.cusTotalpartmaterialCost;
                                list[k].supTotalcost=res.obj.supTotalcost;
                                list[k].supUnitcost=res.obj.supUnitcost;
                                list[k].supTotalpartmaterialCost=res.obj.supTotalpartmaterialCost;
                                list[k].sysTotalcost=res.obj.sysTotalcost;
                                list[k].sysUnitcost=res.obj.sysUnitcost;
                                /*****罗丹珠加----结束 */
                        ...
                        api.getFumoPiceFun(objV).then(res => {
                            console.log(res)
                            if (res.code === 0) {
                                list[k].pourPrice = res.obj.cusUnitcost // 报价总价（客户）
                                list[k].offterTotal = res.obj.cusTotalcost // 复模灌注单价（客户）

                                /****罗丹珠-----开始**** */
                                list[k].proTotal  = res.obj.cusTotalmasterVccost	 // 复模原型总成本（客户)
                                list[k].moldTotal  = res.obj.cusTotalmoldVccost	 // 复模原型总成本（客户)
                                list[k].cusTotalpartmaterialCost  = res.obj.cusTotalpartmaterialCost    //产品总材料费（客户）
                                list[k].supUnitcost  = res.obj.supUnitcost   //单价（供应商）    
                                list[k].supTotalcost  = res.obj.supTotalcost   //总价（供应商）
                                list[k].supTotalmasterVccost  = res.obj.supTotalmasterVccost   //复模原型总成本（供应商）
                                list[k].supTotalpartmaterialCost  = res.obj.supTotalpartmaterialCost   //产品总材料费（供应商）
                                list[k].sysUnitcost  = res.obj.sysUnitcost      //单价（系统）
                                list[k].sysTotalcost  = res.obj.sysTotalcost      //总价（系统）
                                list[k].sysTotalmasterVccost  = res.obj.sysTotalmasterVccost      //复模原型总成本（系统）
                                list[k].sysTotalmoldVccost  = res.obj.sysTotalmoldVccost      //复模硅模总成本（系统）
                                list[k].sysTotalmoldmaterialVccost  = res.obj.sysTotalmoldmaterialVccost      //复模硅模总材料费（系统）
                                list[k].sysTotalpartmaterialCost  = res.obj.sysTotalpartmaterialCost      //产品总材料费（系统）
                                /****罗丹珠-----结束**** */
                        ...
                        api.getModelPiceFun(objV).then(res => {
                            ...
                            if (res.code === 0) {
                                ...
                                /*******罗丹珠----开始********** */
                                list[k].supFmouldMaterialCost=res.obj.supFmouldMaterialCost //快速模具模具总材料费(供应商)
                                list[k].supFmouldMoldTotalcost=res.obj.supFmouldMoldTotalcost   //快速模具总模具费(供应商)
                                list[k].supFmouldMoldcost=res.obj.supFmouldMoldcost   //快速模具单个模具费(供应商)
                                list[k].supFmouldProductionCost=res.obj.supFmouldProductionCost //加工费(供应商)
                                list[k].supFmouldTestmoldCost=res.obj.supFmouldTestmoldCost   //快速模具试模费(供应商)
                                list[k].supTotalcost=res.obj.supTotalcost    //总价(供应商)
                                list[k].supTotalpartmaterialCost=res.obj.supTotalpartmaterialCost    //产品总材料费(供应商)
                                list[k].supUnitcost=res.obj.supUnitcost //快速模具产品单价(供应商)
                                list[k].sysFmouldMoldCost=res.obj.sysFmouldMoldCost //快速模具单个模具费(系统)
                                list[k].sysFmouldDesignCost=res.obj.sysFmouldDesignCost //快速模具设计费（系统）
                                list[k].sysFmouldMaterialCost=res.obj.sysFmouldMaterialCost   //快速模具模具总材料费（系统）
                                list[k].sysFmouldMoldTotalcost=res.obj.sysFmouldMoldTotalcost   //快速模具模具费（系统）
                                list[k].sysFmouldMoldlaborCost=res.obj.sysFmouldMoldlaborCost  //快速模具配模人工费（系统）
                                list[k].sysFmouldMpmaterialCost=res.obj.sysFmouldMpmaterialCost //快速模具模胚材料费（系统）
                                list[k].sysFmouldMrmaterialCost=res.obj.sysFmouldMrmaterialCost //快速模具模仁材料费（系统）
                                list[k].sysFmouldOtherCost=res.obj.sysFmouldOtherCost  //快速模具其他费用（系统）
                                list[k].sysFmouldPjmaterialCost=res.obj.sysFmouldPjmaterialCost //快速模具配件费（系统）
                                list[k].sysFmouldProductionCost=res.obj.sysFmouldProductionCost //加工费（系统）
                                list[k].sysFmouldTestmoldCost=res.obj.sysFmouldTestmoldCost   //快速模具试模费（系统）
                                list[k].sysFmouldTlmaterialCost=res.obj.sysFmouldTlmaterialCost //快速模具铜料费（系统）
                                list[k].sysTotalcost=res.obj.sysTotalcost    //快速模具总价（系统）
                                list[k].sysTotalpartmaterialCost=res.obj.sysTotalpartmaterialCost    //产品总材料费（快速模具胶料费）（系统）
                                list[k].sysUnitcost=res.obj.sysUnitcost //快速模具产品单价（系统）
                                /*******罗丹珠----结束********** */
        ...
        saveFun () {
            ...
            this.tableData0.forEach(item => {
                let obj= {
                   ...
                   /****罗丹珠----开始*** */
                    cusTotalpartmaterialCost:item.cusTotalpartmaterialCost, //产品总材料费(客户)
                    supTotalpartmaterialCost:item.supTotalpartmaterialCost, //产品总材料费(供应商)
                    sysTotalcost:item.sysTotalcost,// 报价单价(系统)
                    sysUnitcost:item.sysUnitcost,// 报价单价(系统)
                    /****罗丹珠----开始*** */
            ...
            this.tableData1.forEach(item => {
                let obj = {
                    ...
                    supTotalcost: item.supTotalcost, // 产品总价（供应商）
                     ...
                     /****罗丹珠----开始*** */
                    sysUnitcost: item.sysUnitcost, // 产品单价（系统）
                    sysTotalcost: item.sysTotalcost // 产品总价（系统）
                     /****罗丹珠----开始*** */
            ...
            this.tableData3.forEach(item => {
                let obj = {
                    ...
                    cusTotalmasterVccost: item.proTotal, // 复模原型总成本
                    cusTotalmoldVccost: item.moldTotal, // 复模硅模总成本
                    cusUnitcost: item.pourPrice, // 报价总价（客户）
                    cusTotalcost: item.offterTotal, // 复模灌注单价（客户）
                    supUnitcost: item.supUnitcost, // 产品单价（供应商）
                    supTotalcost: item.supTotalcost, // 产品总价（供应商）

                    /*****罗丹珠----开始**** */
                    cusTotalpartmaterialCost:item.cusTotalpartmaterialCost,//产品总材料费（客户）
                    supTotalmasterVccost:item.supTotalmasterVccost,    //复模原型总成本（供应商）
                    supTotalpartmaterialCost:item.supTotalpartmaterialCost, //产品总材料费（供应商）
                    sysUnitcost:item.sysUnitcost,   //单价(系统)
                    sysTotalcost:item.sysTotalcost,   //总价(系统)
                    sysTotalmasterVccost:item.sysTotalmasterVccost,//复模原型总成本（系统）
                    sysTotalmoldVccost:item.sysTotalmoldVccost,//复模硅模总成本（系统）
                    sysTotalmoldmaterialVccost:item.sysTotalmoldmaterialVccost,//复模硅模总材料费（系统）
                    sysTotalpartmaterialCost:item.sysTotalpartmaterialCost, //产品总材料费（系统）
                    /*****罗丹珠----结束**** */
            ...
            this.tableData4.forEach(item => {
                let obj = {
                    ...
                    sysTotalpartmaterialCost:item.sysTotalpartmaterialCost,//快速模具胶料费
                    sysUnitcost:item.sysUnitcost,   //单价(系统)
                    sysTotalcost:item.sysTotalcost, //总价(系统)
        ...
        amountHandleChange3d (val, type) {
            ...
            if (type === 'tab0') {
                api.getThrPirceFun(obj).then(res => {
                    console.log("获取3D价格数据====1=====")
                    if (res.code === 0) {
                        val.offterPrice = res.obj.cusUnitcost
                        val.offterTotal = res.obj.cusTotalcost

                        /*****罗丹珠加----开始 */
                        val.cusTotalpartmaterialCost=res.obj.cusTotalpartmaterialCost;
                        val.supTotalcost=res.obj.supTotalcost;
                        val.supUnitcost=res.obj.supUnitcost;
                        val.supTotalpartmaterialCost=res.obj.supTotalpartmaterialCost;
                        val.sysTotalcost=res.obj.sysTotalcost;
                        val.sysUnitcost=res.obj.sysUnitcost;
                        /*****罗丹珠加----结束 */
        ...
        // 修改3D打印总价是 调用的方法
        eidtTotalPirce (type) {
            console.log("====eidtTotalPirce修改3D打印总价是 调用的方法======")
            ...
            if (type === 'tab0') {
                ...
                api.getModelPiceFun(obj).then(res => {
                    if (res.code === 0) {
                        ...
                        /*******罗丹珠----开始********** */
                        val.supFmouldMaterialCost=res.obj.supFmouldMaterialCost //快速模具模具总材料费(供应商)
                        val.supFmouldMoldTotalcost=res.obj.supFmouldMoldTotalcost   //快速模具总模具费(供应商)
                        val.supFmouldMoldcost=res.obj.supFmouldMoldcost   //快速模具单个模具费(供应商)
                        val.supFmouldProductionCost=res.obj.supFmouldProductionCost //加工费(供应商)
                        val.supFmouldTestmoldCost=res.obj.supFmouldTestmoldCost   //快速模具试模费(供应商)
                        val.supTotalcost=res.obj.supTotalcost    //总价(供应商)
                        val.supTotalpartmaterialCost=res.obj.supTotalpartmaterialCost    //产品总材料费(供应商)
                        val.supUnitcost=res.obj.supUnitcost //快速模具产品单价(供应商)
                        val.sysFmouldMoldCost=res.obj.sysFmouldMoldCost //快速模具单个模具费(系统)
                        val.sysFmouldDesignCost=res.obj.sysFmouldDesignCost //快速模具设计费（系统）
                        val.sysFmouldMaterialCost=res.obj.sysFmouldMaterialCost   //快速模具模具总材料费（系统）
                        val.sysFmouldMoldTotalcost=res.obj.sysFmouldMoldTotalcost   //快速模具模具费（系统）
                        val.sysFmouldMoldlaborCost=res.obj.sysFmouldMoldlaborCost  //快速模具配模人工费（系统）
                        val.sysFmouldMpmaterialCost=res.obj.sysFmouldMpmaterialCost //快速模具模胚材料费（系统）
                        val.sysFmouldMrmaterialCost=res.obj.sysFmouldMrmaterialCost //快速模具模仁材料费（系统）
                        val.sysFmouldOtherCost=res.obj.sysFmouldOtherCost  //快速模具其他费用（系统）
                        val.sysFmouldPjmaterialCost=res.obj.sysFmouldPjmaterialCost //快速模具配件费（系统）
                        val.sysFmouldProductionCost=res.obj.sysFmouldProductionCost //加工费（系统）
                        val.sysFmouldTestmoldCost=res.obj.sysFmouldTestmoldCost   //快速模具试模费（系统）
                        val.sysFmouldTlmaterialCost=res.obj.sysFmouldTlmaterialCost //快速模具铜料费（系统）
                        val.sysTotalcost=res.obj.sysTotalcost    //快速模具总价（系统）
                        val.sysTotalpartmaterialCost=res.obj.sysTotalpartmaterialCost    //产品总材料费（快速模具胶料费）（系统）
                        val.sysUnitcost=res.obj.sysUnitcost //快速模具产品单价（系统）
                        /*******罗丹珠----结束********** */
