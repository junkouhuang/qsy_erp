...
export default {
  ...
  data() {
    return {
      menuId:'7ecbb514-c414-11e9-8631-000c290d53bf',//品鉴menuId
      dialogReport:false,//品检报告
      dialogImageUrl: '',
      dialogVisible: false,
      radio:1,
      tableData: [{
      ...
      tableDataOne: [{QCNo:'HJJJJJJAJSJA'}], //一级表数据
      tableDataTwo: [{QCNo:'22222'}], //二级表数据
      tableDataThree: [{QCNo:'22222'}], //三级表数据
      ...
      dialogQuo: false,
      dialogView:false,
      dialogRevisionOrder:false,    //转订单
      dialogNoQuote:false,          //转未成交
      dialogAtt:false,
      boolDoll:false,     //公仔
      ...
      currencyRow:[],     //当前行
      currencyStatus:'',    //当前要保存的状态
      dialogRevisionOrder:false,
      dialogNoOrder:false,
      dialogSong:false,//送货dialog
      quotationNoOrder:'',      //要转成订单的报价单号
      quotationOrderCost:'',    //要转成订单的总金额
      projectPmUser:false,      //是否显示项目跟单列
      listProjectPmUser:'' ,     //项目跟单员列表
      projectPmUserCode:'',       //项目跟单员编号
      listNoDealReason:[],      //未成交原因列表
      noDealReason:'',          //未成交原因
      noDealDetails:'',          //未成交备注
      noDealSolution:'',          //解决方案
  ...
  mounted () {
    ...
    this.getStatusList()
    this.getButtonsList();
    //this.getOneTableData(this.currentPage,this.pageSize,"2")
    //this.getPoList(this.currentPage,this.pageSize,"2");
  },
  methods: {
    /**--打开dialog--**/
    openSongHuo(){
      console.log("送货dialog");
      this.dialogSong = true;
    },
    /**--获取select文本输入start--**/
    setPoNo(val){
      this.ruleForm.poNo = val;
    },
    setProjectName(val){
      this.ruleForm.projectName = val;
    },
    setOrderNo(val){
      this.ruleForm.orderNo = val;
    },
    /**--获取select文本输入end--**/
    ...
    getButtonsList(){
      this.$request.post('/sysOperation/listOperationBar', {menuId: this.menuId}).then(res => {
        ...
        if (res.code == 0) {
          ...
          this.threeOperation=res.obj.listThreeTableOperation;
          this.toolsList=res.obj.listToolBarOperation;
          console.log(this.threeOperation)
        }
      }).catch(e=>{
        console.log("错误")
      })
    ...
    getOneTableData(currentPage,pageSize,searchType){
      this.$request.post('/pmpomain/listPoMain', {
        ...
        page:this.currentPage,
        rows:this.pageSize,
        menuId:'7ecbb37e-c414-11e9-8631-000c290d53bf',
        serachType:2,//1为精确查找，2位模糊查找
        poNo:this.ruleForm.poNo,//外发单号
       // projectName:this.ruleForm.projectName,//产品名称
        orderNo: this.ruleForm.orderNo, //订单编号
        dateType: this.ruleForm.chooses, //日期查询类型（0订单交期 1生产交期）
        starts: this.ruleForm.start,//起始时间
        ends:this.ruleForm.end,//结束时间
        statusCode: this.ruleForm.rating//状态查询
      }).then(res => {
        console.log("获取一级表数据")
        console.log(res)    //请求成功方法，data为请求成功返回的数据
        if (res.code == 0) {
   ...
   /* getPoList(){
    ...
    /**获取模具跟单员 */
    getMoldProjectUser(){
      this.$request.post('/hremployeeinfo/listEmployeeProjectUserByDeptCode', {
        deptCode: '1402'
      }).then(res => {
        console.log(res)    //请求成功方法，data为请求成功返回的数据
        if (res.code == 0) {
          this.listProjectPmUser=res.obj;
        }
      }).catch(e=>{
        this.$message({
          type: 'error',
    ...
    /**转订单*/
    revisionOrder(index, row,status){
      this.projectPmUserCode="";
      this.getMoldProjectUser();

      setTimeout(() => {
        /**如果是模具 */
        if(row.quotationType=='2'){
          this.projectPmUser=true;
        }else{
          this.projectPmUser=false;
        }
        this.dialogTitle = '转订单'
        this.currencyRow=row;       //当前行
        this.currencyStatus=status;    //当前要保存的状态
        this.quotationNoOrder=row.quotationNo;
        this.quotationOrderCost=row.totalCost;
        this.dialogRevisionOrder = true
      },300)
    },

    /**转订单-----保存 */
    submitDialogDataOrder(){
      /*更改报价单状态*/
      this.sender('123456',this.currencyRow,this.currencyStatus)
      /**添加到订单主表、明细表、附件表 */
      this.$request.post('/salordmain/quoteVersionOrder', {
        ids: this.currencyRow.id,
        projectPmUsercode:this.projectPmUserCode
      }).then(res => {
        console.log(res)    //请求成功方法，data为请求成功返回的数据
        if (res.code == 0) {
          this.$message({
      ...
      /**关闭对话框 */
      this.dialogRevisionOrder = false;
    },

    /**获取未成交原因列表 */
    getNoDealResonList(){
      this.$request.post('/sysbasicinfo/listBasicInfoByType', {
        categoryType: 'NDR'
      }).then(res => {
        console.log(res)    //请求成功方法，data为请求成功返回的数据
        if (res.code == 0) {
          this.listNoDealReason=res.obj;
        }
      }).catch(e=>{
        this.$message({
          type: 'error',
    ...
    /**转未成交 */
    changedealnoClick(index, row,status){
      this.getNoDealResonList();
      this.noDealReason=""
      /**弹框 */
      setTimeout(() => {
        this.currencyRow=row;       //当前行
        this.currencyStatus=status;    //当前要保存的状态
        this.dialogTitle = '未成交原因收集'
        this.dialogNoOrder = true
      },300)
    ...
    submitDialogOrder(){
      ...
      /*更改报价单状态*/
      this.sender("1234",this.currencyRow,this.currencyStatus)

      /**保存 */
      this.$request.post('/salqtmain/saveNoDealQuote', {
        "ids":this.currencyRow.id,
        "noDealReason":this.noDealReason,
        "noDealDetails":this.noDealDetails,
        "noDealSolution":this.noDealSolution
      }).then(res => {
        console.log(res)    //请求成功方法，data为请求成功返回的数据
        if (res.code == 0) {
          this.$message({
