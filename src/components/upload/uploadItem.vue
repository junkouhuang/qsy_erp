<template>
  <div class="upload">
    ...
    <div id="ossfile5" v-show="flag"></div>
    <div id="container5">
      <p :id='msg23'  href="javascript:void(0);" class='btn'  @mousedown="getsign">上传附件</p>
      <a id="postfiles5" href="javascript:void(0);" class='btn' v-show="flag">开始上传</a>
      <!-- <img src="../../assets/image/info_def.png" alt="暂无图片" :id='msg2' href="javascript:void(0);" class='btn'  @mousedown="getsign" /> -->
    </div>
    <!--提示语-->
...
<script>
  ...
  export default {
    ...
    methods: {
      getsign () {
        this.msg23="iii_ooo"
        // 'http://192.168.10.14:9259/admin/oss/oss/uploadFile', {paraCode: 'OSSQuoteDM'}
        // 'http://47.106.92.209:8087/qt-admin/quotation/upload', {}
        // let xiosUrl = "http://192.168.10.14:9259/admin/oss/oss/uploadFile', {paraCode: 'OSSQuoteDM'}"
        this.$request.post('/oss/oss/uploadFile', {paraCode: 'OSSQuoteDF'})
          .then((response) => {
              ...
              var uploader = new plupload.Uploader({
                ...
                filters: {
                  mime_types: [ //只允许上传图片和zip文件和stl文件
                    ...
                    {title: "Image files", extensions: "jpeg,png,jpg,bmp,gif,rar,zip,RAR,ZIP,JPG,JPEG,PNG,BMP"},
                    {title: 'Zip files', extensions: 'zip, ZIP, Zip'},
                    {title: 'Rar files', extensions: 'rar, RAR, Rar'},
                    {title: "THREE files", extensions: "stl,STL,igs,IGS,IGES,iges,step,stp,IGES,STP,STEP"}
                  ],
                  max_file_size: '1000mb', //最大只能上传100mb的文件
                ...
                init: {
                  PostInit:()=> {
                    ...
                    document.getElementById('ossfile5').innerHTML = '';
                    document.getElementById('postfiles5').onclick = function () {
                  ...
                  FilesAdded:  (up, files)=> {
                    ...
                    plupload.each(files,  (file)=> {
                      document.getElementById('ossfile5').innerHTML += '<div id="' + file.id + '">' + file.name + ' (' + plupload.formatSize(file.size) + ')<b></b>'
                  ...
                  FileUploaded:  (up, file, info) =>{
                    ...
                    if (info.status == 200) {
                      ...
                      var ossName = this.get_uploaded_object_name(file.name)
                      //将路径添加到
                      this.$emit('getossurl',ossPath,igsCanvasId,file, ossName);
